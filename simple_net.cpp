...
void simple_net(int times = 100) {
  ...
  for (int i = 0; i < 1; ++i) {
    ...
    std::vector<float> net_dst(batch * 1000);
    ...
    std::vector<float> conv1_weights(std::accumulate(conv1_weights_tz.begin(),
                                                     conv1_weights_tz.end(), 1, std::multiplies<uint32_t>()));
    std::vector<float> conv1_bias(std::accumulate(conv1_bias_tz.begin(),
                                                  conv1_bias_tz.end(), 1, std::multiplies<uint32_t>()));

    /* create memory for user data */
    auto conv1_user_src_memory = memory({{{conv1_src_tz}, memory::data_type::f32,
                                          memory::format::nchw}, cpu_engine}, net_src.data());
    auto conv1_user_weights_memory = memory({{{conv1_weights_tz},
                                              memory::data_type::f32, memory::format::oihw}, cpu_engine},
                                            conv1_weights.data());
    auto conv1_user_bias_memory = memory({{{conv1_bias_tz},
                                           memory::data_type::f32, memory::format::x}, cpu_engine},
                                         conv1_bias.data());

    /* create memory descriptors for convolution data w/ no specified format */
    auto conv1_src_md = memory::desc({conv1_src_tz}, memory::data_type::f32,
                                     memory::format::any);
    auto conv1_bias_md = memory::desc({conv1_bias_tz}, memory::data_type::f32,
                                      memory::format::any);
    auto conv1_weights_md = memory::desc({conv1_weights_tz},
                                         memory::data_type::f32, memory::format::any);
    auto conv1_dst_md = memory::desc({conv1_dst_tz}, memory::data_type::f32,
                                     memory::format::any);
    ...
    if (memory::primitive_desc(conv1_prim_desc.src_primitive_desc()) !=
      ...
      net.push_back(reorder(conv1_user_src_memory, conv1_src_memory));
    }
    ...
    net.push_back(convolution_forward(conv1_prim_desc, conv1_src_memory,
                                      conv1_weights_memory, conv1_user_bias_memory, conv1_dst_memory));
    ...
    */
    ...
    std::vector<float> conv2_weights(std::accumulate(conv2_weights_tz.begin(),
                                                     conv2_weights_tz.end(), 1, std::multiplies<uint32_t>()));
    ...
    */
    ...
    std::vector<float> conv3_weights(std::accumulate(conv3_weights_tz.begin(),
                                                     conv3_weights_tz.end(), 1, std::multiplies<uint32_t>()));
    std::vector<float> conv3_bias(std::accumulate(conv3_bias_tz.begin(),
                                                  conv3_bias_tz.end(), 1, std::multiplies<uint32_t>()));

    /* create memory for user data */
    auto conv3_user_weights_memory = memory({{{conv3_weights_tz},
                                              memory::data_type::f32, memory::format::oihw}, cpu_engine},
                                            conv3_weights.data());
    auto conv3_user_bias_memory = memory({{{conv3_bias_tz},
                                           memory::data_type::f32, memory::format::x}, cpu_engine},
                                         conv3_bias.data());
    ...
    auto conv3_weights_md = memory::desc({conv3_weights_tz},
                                         memory::data_type::f32, memory::format::any);
    auto conv3_dst_md = memory::desc({conv3_dst_tz}, memory::data_type::f32,
                                     memory::format::any);
    ...
    */
    ...
    std::vector<float> conv4_weights(std::accumulate(conv4_weights_tz.begin(),
                                                     conv4_weights_tz.end(), 1, std::multiplies<uint32_t>()));
    std::vector<float> conv4_bias(std::accumulate(conv4_bias_tz.begin(),
                                                  conv4_bias_tz.end(), 1, std::multiplies<uint32_t>()));

    /* create memory for user data */
    auto conv4_user_weights_memory = memory({{{conv4_weights_tz},
                                              memory::data_type::f32, memory::format::goihw}, cpu_engine},
                                            conv4_weights.data());
    auto conv4_user_bias_memory = memory({{{conv4_bias_tz},
                                           memory::data_type::f32, memory::format::x}, cpu_engine},
                                         conv4_bias.data());
    ...
    auto conv4_weights_md = memory::desc({conv4_weights_tz},
                                         memory::data_type::f32, memory::format::any);
    auto conv4_dst_md = memory::desc({conv4_dst_tz}, memory::data_type::f32,
                                     memory::format::any);
    ...
    */
    ...
    std::vector<float> conv5_weights(std::accumulate(conv5_weights_tz.begin(),
                                                     conv5_weights_tz.end(), 1, std::multiplies<uint32_t>()));
    std::vector<float> conv5_bias(std::accumulate(conv5_bias_tz.begin(),
                                                  conv5_bias_tz.end(), 1, std::multiplies<uint32_t>()));

    /* create memory for user data */
    auto conv5_user_weights_memory = memory({{{conv5_weights_tz},
                                              memory::data_type::f32, memory::format::goihw}, cpu_engine},
                                            conv5_weights.data());
    auto conv5_user_bias_memory = memory({{{conv5_bias_tz},
                                           memory::data_type::f32, memory::format::x}, cpu_engine},
                                         conv5_bias.data());

    /* create memory descriptors for convolution data w/ no specified format */
    auto conv5_bias_md = memory::desc({conv5_bias_tz}, memory::data_type::f32,
                                      memory::format::any);
    auto conv5_weights_md = memory::desc({conv5_weights_tz},
                                         memory::data_type::f32, memory::format::any);
    auto conv5_dst_md = memory::desc({conv5_dst_tz}, memory::data_type::f32,
                                     memory::format::any);
    ...
    */
    ...
    /* create memory for user data */
    auto fc6_user_weights_memory = memory({{{fc6_weights_tz},
                                            memory::data_type::f32, memory::format::oihw}, cpu_engine},
                                          fc6_weights.data());

    auto fc6_user_bias_memory = memory({{{fc6_bias_tz},
                                         memory::data_type::f32, memory::format::x}, cpu_engine},
                                       fc6_bias.data());
    ...
    /* create memory for user data */
    auto fc7_user_weights_memory = memory({{{fc7_weights_tz},
                                            memory::data_type::f32, memory::format::nc}, cpu_engine},
                                          fc7_weights.data());

    auto fc7_user_bias_memory = memory({{{fc7_bias_tz},
                                         memory::data_type::f32, memory::format::x}, cpu_engine},
                                       fc7_bias.data());

    /* create memory descriptors for convolution data w/ no specified format */
    auto fc7_bias_md = memory::desc({fc7_bias_tz}, memory::data_type::f32, memory::format::any);
    auto fc7_weights_md = memory::desc({fc7_weights_tz}, memory::data_type::f32, memory::format::any);
    ...
    /* create convolution primitive and add it to net */
    net.push_back(inner_product_forward(fc7_prim_desc, fc6_dst_memory, fc7_weights_memory, fc7_user_bias_memory,
                                        fc7_dst_memory));
    ...
    */
    ...
    /* create memory for user data */
    auto fc8_user_weights_memory = memory({{{fc8_weights_tz},
                                            memory::data_type::f32, memory::format::nc}, cpu_engine},
                                          fc8_weights.data());

    auto fc8_user_bias_memory = memory({{{fc8_bias_tz},
                                         memory::data_type::f32, memory::format::x}, cpu_engine},
                                       fc8_bias.data());

    auto net_dst_memory = memory({{{fc8_dst_tz}, memory::data_type::f32,
                                   memory::format::nc}, cpu_engine}, net_dst.data());

    /* create memory descriptors for convolution data w/ no specified format */
    auto fc8_bias_md = memory::desc({fc8_bias_tz}, memory::data_type::f32, memory::format::any);
    auto fc8_weights_md = memory::desc({fc8_weights_tz}, memory::data_type::f32, memory::format::any);
    ...
    /* create convolution primitive and add it to net */
    net.push_back(inner_product_forward(fc8_prim_desc, fc7_dst_memory, fc8_weights_memory, fc8_user_bias_memory,
                                        fc8_dst_memory));

    /* create reorder between internal and user data if it is needed and
     *  add it to net after pooling */
    if (fc8_dst_memory != net_dst_memory) {
